    ...
    try {
      if (process.platform === 'win32') {
        ...
        try {
          ...
          safeLog('⚠️ [WINDOW-TITLE] All methods failed:', (windowError as Error).message?.substring(0, 200));
        }
  ...
  try {
    ...
    if (!finalURL && appName.includes('Chrome')) {
      ...
      try {
        ...
        safeLog('❌ [URL-FALLBACK] External Chrome script failed:', (error as Error).message);
      }
    }
