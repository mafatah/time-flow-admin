...
exports.startIndependentAppTracking = startIndependentAppTracking;
exports.stopIndependentAppTracking = stopIndependentAppTracking;
exports.startIndependentURLTracking = startIndependentURLTracking;
exports.stopIndependentURLTracking = stopIndependentURLTracking;
...
// Intervals
let batchUploadInterval = null;
let screenshotScheduleTimeout = null;
let immediateDetectionInterval = null;
...
async function startImmediateDetection() {
    ...
    immediateDetectionInterval = setInterval(async () => {
        ...
        try {
            ...
            safeLog('⚠️ Immediate detection failed:', error.message);
        }
...
async function storeAppChangeLocally(appName, windowTitle) {
    const appData = {
...
async function storeURLChangeLocally(url, browser) {
    const urlData = {
        user_id: currentUserId,
        project_id: '00000000-0000-0000-0000-000000000001',
        url: url,
...
async function uploadLocalQueues() {
    ...
    if (appCount > 0) {
        try {
            const { error: appError } = await supabase_1.supabase
                .from('app_logs')
    ...
    if (urlCount > 0) {
        try {
            const { error: urlError } = await supabase_1.supabase
                .from('url_logs')
    ...
    try {
        ...
        const screenshotPromise = new Promise(async (resolve, reject) => {
            try {
                ...
                const tempPath = path.join(electron_1.app.getPath('temp'), filename);
        ...
        const { tempPath, filename, context } = await Promise.race([screenshotPromise, timeoutPromise]);
        // Upload with context
...
async function checkAndUpdatePermissions() {
    ...
    hasScreenRecordingPermission = await (0, permissionManager_1.checkScreenRecordingPermission)();
        ...
        if (process.platform === 'darwin') {
            ...
            if (!activeWinDisabled && await checkAndUpdatePermissions()) {
                try {
                    ...
                    safeLog('❌ Active-win failed, falling back to AppleScript:', e.message);
                }
            }
        ...
        if (process.platform === 'darwin') {
            ...
            if (!activeWinDisabled && await checkAndUpdatePermissions()) {
                try {
                    ...
                    safeLog('❌ Active-win window title failed, falling back to AppleScript:', e.message);
                }
            }
    ...
    try {
        ...
        if (process.platform === 'darwin' && !activeWinDisabled && await checkAndUpdatePermissions()) {
            try {
                ...
                safeLog('❌ Active-win URL detection failed, falling back to AppleScript:', e.message);
            }
        }
...
async function startIndependentAppTracking() {
    ...
    independentAppInterval = setInterval(async () => {
        ...
        try {
            ...
            safeLog('⚠️ App tracking failed:', error.message);
        }
...
async function saveAppLogEntry(appLogData) {
    try {
...
async function startIndependentURLTracking() {
    ...
    independentUrlInterval = setInterval(async () => {
        ...
        try {
            ...
            safeLog('⚠️ URL tracking failed:', error.message);
        }
...
async function saveURLLogEntry(urlLogData) {
    try {
        const { error } = await supabase_1.supabase
            .from('url_logs')
            .insert(urlLogData);
...
async function uploadActivityScreenshot(filePath, filename, context) {
    if (!currentUserId) {
...
async function triggerActivityCapture() {
    if (!currentUserId) {
        // Use the same robust recovery as recordRealActivity
        const defaultUserId = '0c3d3092-913e-436f-a352-3378e558c34f';
        currentUserId = defaultUserId;
...
function recordRealActivity(activityType, count) {
    ...
    if (!currentUserId) {
        ...
        try {
            const { getUserId } = require('./tracker.cjs');
            ...
            safeLog('⚠️ Layer 1 recovery failed:', error.message);
        }
        ...
        if (!currentUserId) {
            try {
                const { loadSession } = require('./sessionManager.cjs');
                ...
                safeLog('⚠️ Layer 4 recovery failed:', error.message);
            }
        }
