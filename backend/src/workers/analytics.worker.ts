...
export class AnalyticsWorker {
  ...
  // ITEM 12: Suspicious Activity Detector Job - Runs every 15 minutes
  @Cron('*/15 * * * *')
  async detectSuspiciousActivity() {
    this.logger.log('🚨 Starting suspicious activity detection job...');
    ...
    try {
      ...
      for (const user of users) {
        try {
          const detections = await this.analyzeUserSuspiciousActivity(user.id, last30Minutes, now);
      ...
      this.logger.error('❌ Suspicious activity detection job failed:', error);
    }
  }
  ...
  private async analyzeUserSuspiciousActivity(userId: string, startDate: Date, endDate: Date) {
    const supabase = this.supabaseService.getClient();
    const detections = [];
